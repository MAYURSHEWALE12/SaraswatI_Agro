...
export default function QualityAssurance() {
  ...
  return (
    <section className="py-16 sm:py-20 md:py-28 bg-white dark:bg-slate-950 transition-colors overflow-hidden">
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          <span className="inline-block bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 text-sm font-semibold px-4 py-1.5 rounded-full mb-4 transition-colors">
            {t({ mr: "🏅 गुणवत्ता", en: "🏅 Quality" })}
          </span>
          <h2 className="text-3xl md:text-5xl font-bold text-gray-800 dark:text-white mb-4 transition-colors">
            {t({ mr: "गुणवत्तेची ", en: "Quality " })}<span className="text-emerald-600 dark:text-emerald-400">{t({ mr: "हमी", en: "Assurance" })}</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-2 gap-10 md:gap-14 items-center">
            ...
            {qualityPoints.map((point, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex flex-col gap-3 p-5 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-lg rounded-2xl transition-all duration-300 group cursor-default"
                  data-testid={`card-quality-${i}`}
                >
                  <div className="w-11 h-11 bg-emerald-50 dark:bg-slate-800 rounded-xl flex items-center justify-center text-xl group-hover:bg-emerald-600 dark:group-hover:bg-emerald-500 group-hover:scale-110 transition-all duration-300">
          ...
          >
            ...
            <div className="absolute -top-4 -right-4 w-20 h-20 bg-emerald-100 dark:bg-emerald-900/20 rounded-full blur-2xl -z-10 transition-colors" />
            <div className="absolute -bottom-4 -left-4 w-28 h-28 bg-amber-100 dark:bg-amber-900/20 rounded-full blur-2xl -z-10 transition-colors" />
          </motion.div>
        </div>
      </div>
    </section>
